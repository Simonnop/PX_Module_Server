...
"""
功能：测试调用后端接口 `GET /module/online` 并打印返回结果。

使用方式：
  python _client/test_show_online_modules.py \
...
def build_url(base_url: str) -> str:
    """拼接接口完整 URL。"""
    return base_url.rstrip("/") + "/module/online"
...
def fetch(url: str, timeout_seconds: float) -> bytes:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ForecastClient/1.0 (+show_online_modules)",
        },
    )
    context = None
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="调用 /module/online 接口并打印结果")
    parser.add_argument(
        "--base-url",
