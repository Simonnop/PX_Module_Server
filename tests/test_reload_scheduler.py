...
"""
功能：测试调用后端接口 `POST /scheduler/reload` 重新加载调度器任务。

使用方式：
  python tests/test_reload_scheduler.py \
...
def build_url(base_url: str) -> str:
    """拼接接口完整 URL。"""
    return base_url.rstrip("/") + "/scheduler/reload"
...
def fetch(url: str, timeout_seconds: float) -> bytes:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ForecastClient/1.0 (+reload_scheduler)",
        },
    )
    context = None
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="调用 /scheduler/reload 接口并打印结果")
    parser.add_argument(
        "--base-url",
    ...
    # 简单结构断言：成功时 result 应为字典
    if code == "2000" and not isinstance(result, dict):
        print("期望 result 为字典", file=sys.stderr)
        sys.exit(3)
